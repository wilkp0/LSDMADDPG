...
class Buildings:
    ...
    def building_loader(data_path, building_attributes, weather_file, building_ids, buildings_states_actions, simulation_period, deterministic, save_memory = True):
        ...
        shared_observation_space = spaces.Box(low=np.array(s_low), high=np.array(s_high), dtype=np.float32)
        shared_action_space = spaces.Box(low=np.array(a_low), high=np.array(a_high), dtype=np.float32)
        
        for building in buildings.values():
            building.observation_space = shared_observation_space
            building.action_space = shared_action_space
            building.reset()

        return buildings, shared_observation_space, shared_action_space
