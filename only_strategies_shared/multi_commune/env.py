...
class Community(gym.Env):
    def __init__(self, data_path, building_attributes, weather_file, building_ids,  buildings_states_actions = None, simulation_period = (0,8759), deterministic =False, cost_function = ['ramping','1-load_factor','average_daily_peak','peak_demand','net_electricity_consumption', 'net_electricity_cost','net_comfort_penalty'], save_memory = True, verbose = 0):
        ...
        self.buildings, self.observation_space, self.action_space, self.strategy_space = Buildings.building_loader(**params_loader)

        print("OBSSPACE", self.observation_space)
        print("STRSPACE", self.strategy_space)
    ...
    def step(self, actions):
        ...
        for a, (uid, building) in zip(actions, self.buildings.items()):
            ...
            #self.ramping = building.set_ramping()
            #self.results_ramping[uid].append(self.ramping)
            ...
            for state_name, value in self.buildings_states_actions[uid]['states'].items():
                if value == True:
                    ...
                    if state_name == "t_in":
                        ...
                        self.strategies.append(self.get_strategies(self.buildings, peak))
                        ''' Other States '''
        ...
        self.strategies = np.array(self.strategies, dtype='object')
        ...
        return (self._get_ob(), rewards, self.terminal, {}, self.strategies)
    ...
    def reset(self, eval=False):
        ...
        for uid, building in self.buildings.items():
            ...
            #self.strategies.append(np.array(strategy))
        self.state = np.array(self.state, dtype='object')
        self.strategies = np.array(self.strategies, dtype='object')

        return self._get_ob(), self.strategies
    ...
    def get_strategies(self, buildings , peak):
        strategy = [] 
        for building in buildings.values():
            community_demand  = peak 
            max_peak = 17 * 3  # + 10* 3 # Maximum peak value
            b_soc = building.charging_station.soc
            in_temp = building.hvac.indoor_temp
            # Dynamically adjust weights based on SOC and community demand
            soc_target = 94  # Target SOC level
            soc_range = 20  # SOC range for adjustments
            w_soc = max(0.1, 1 - b_soc/100)  # Adjust weight based on SOC proximity to target
            strategy.append(w_soc)
            #w_temp = 1 if in_temp > 75 or in_temp < 67 else 0.1
            #strategy.append(w_temp)
        return strategy
        ...
        for index, (uid, building) in enumerate(self.buildings.items()):
            ...
            for s_name, s in self.buildings_states_actions[uid]['states'].items():
                if not s:
                    ...
                    state_n += 1
                elif s_name == "strategy":
                    pass
                    # Append the normalized strategy state for n_agents * n_strategies
                #    normalized_value = normalize(obs_space.low[state_n], obs_space.high[state_n])
                #    strategy_vector = [normalized_value] * n_strategies
                #    self.encoder[index].extend(strategy_vector)
                #    state_n += 1
                else:
    ...
    def get_final_results(self):
        ...
        with pd.ExcelWriter('final_results_com_strats_week_figuringouterror.xlsx', engine='openpyxl') as writer:
            for uid in self.buildings.keys():
        ...
        print("Results have been exported to final_results_com_strats_week.xlsx")
