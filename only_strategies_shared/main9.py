...
def run(config):
    ...
    replay_buffer = ReplayBuffer(config.buffer_length, maddpg.nagents,
                             ...
                             [sdim.shape[0] for sdim in env.strategy_space])  # Assuming env.strategy_space contains strategy dimensions
    ...
    for ep_i in range(0, config.n_episodes, config.n_rollout_threads):
        if ep_i % 100 ==0:
            print("Episodes %i-%i of %i" % (ep_i + 1,
                                            ...
                                            config.n_episodes))
        obs, strat = env.reset(eval=False)
        ...
        for et_i in range(config.episode_length):
            ...
            # Concatenate state and strategy for each agent
            ...
            next_obs, rewards, dones, infos, next_strat = env.step(actions)
            replay_buffer.push(obs, agent_actions, rewards, next_obs, dones, strat, next_strat)
            obs = next_obs
            strat = next_strat
        ...
        if ep_i % config.eval_interval < config.n_rollout_threads and ep_i != 0:
            ...
            for t_i in range(config.episode_length-1):
                ...
                agent_actions = [ac.data.numpy() for ac in torch_agent_actions]
                actions = [[ac[i] for ac in agent_actions] for i in range(config.n_rollout_threads)]
                obs, rewards, dones, infos, strat = env.step(actions)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_episodes", default=400000, type=int)
    ...
    parser.set_defaults(strategy=True)  # Set default value to True
