...
def run(config):
    ...
    for ep_i in range(config.n_episodes):
        ...
        for t_i in range(config.episode_length):
            ...
            torch_obs = [Variable(torch.Tensor(np.array(obs[i], dtype=np.float32)).view(1, -1),
                        requires_grad=False)
            ...
            obs, rewards, dones, infos, next_strat = env.step(actions)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--run_num", default=3, type=int, help="Run number")
